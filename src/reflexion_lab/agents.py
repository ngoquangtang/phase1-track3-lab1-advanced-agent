from __future__ import annotations
from dataclasses import dataclass
from typing import Literal
import time
from .llm_runtime import actor_answer, evaluator, reflector
from .schemas import AttemptTrace, QAExample, ReflectionEntry, RunRecord

@dataclass
class BaseAgent:
    agent_type: Literal["react", "reflexion"]
    max_attempts: int = 1
    def run(self, example: QAExample) -> RunRecord:
        reflection_memory: list[str] = []
        reflections: list[ReflectionEntry] = []
        traces: list[AttemptTrace] = []
        final_answer = ""
        final_score = 0
        
        for attempt_id in range(1, self.max_attempts + 1):
            # 1. Gọi Actor để lấy câu trả lời
            answer, actor_tokens, actor_latency = actor_answer(example, attempt_id, self.agent_type, reflection_memory)
            
            # 2. Gọi Evaluator để chấm điểm
            judge, eval_tokens, eval_latency = evaluator(example, answer)
            
            total_attempt_tokens = actor_tokens + eval_tokens
            total_attempt_latency = actor_latency + eval_latency
            
            final_answer = answer
            final_score = judge.score
            
            # TODO: Logic Reflexion tại đây
            if self.agent_type == "reflexion" and judge.score == 0 and attempt_id < self.max_attempts:
                # 3. Gọi reflector để lấy nội dung reflection
                reflection, ref_tokens, ref_latency = reflector(example, attempt_id, judge)
                
                # 4. Cập nhật reflection_memory để Actor dùng cho lần sau
                reflections.append(reflection)
                reflection_memory.append(reflection.next_strategy)
                
                total_attempt_tokens += ref_tokens
                total_attempt_latency += ref_latency

            trace = AttemptTrace(
                attempt_id=attempt_id, 
                answer=answer, 
                score=judge.score, 
                reason=judge.reason, 
                token_estimate=total_attempt_tokens, 
                latency_ms=total_attempt_latency
            )
            traces.append(trace)

            if judge.score == 1:
                break
                
        total_tokens = sum(t.token_estimate for t in traces)
        total_latency = sum(t.latency_ms for t in traces)
        
        failure_mode = "none" if final_score == 1 else "wrong_final_answer"
        
        return RunRecord(
            qid=example.qid, 
            question=example.question, 
            gold_answer=example.gold_answer, 
            agent_type=self.agent_type, 
            predicted_answer=final_answer, 
            is_correct=bool(final_score), 
            attempts=len(traces), 
            token_estimate=total_tokens, 
            latency_ms=total_latency, 
            failure_mode=failure_mode, 
            reflections=reflections, 
            traces=traces
        )

class ReActAgent(BaseAgent):
    def __init__(self) -> None:
        super().__init__(agent_type="react", max_attempts=1)

class ReflexionAgent(BaseAgent):
    def __init__(self, max_attempts: int = 3) -> None:
        super().__init__(agent_type="reflexion", max_attempts=max_attempts)
